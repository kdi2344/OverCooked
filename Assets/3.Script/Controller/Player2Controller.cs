...
public class Player2Controller : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.RightShift) && activeObject != null) //사용 가능한 물체가 있고 Space 눌렀다면
        {
            ...
            else if (activeObjectOb.type == Object.ObjectType.Craft) //Crate와 상호작용
            {
                ...
                {
                    activeObject.GetComponent<Craft>().OpenCraftPlayer2(); //꺼내기
                    activeObjectOb.onSomething = false;
                    isHolding = true;
                    anim.SetBool("isHolding", isHolding);
                }
        ...
        {//자르기
            if (activeObject.transform.GetChild(0).GetComponent<CuttingBoard>()._CoTimer == null) //한번도 실행 안된거면 시작 가능
            {
                ...
                activeObject.transform.GetChild(0).GetComponent<CuttingBoard>().StartCutting2();
            }
    ...
    void FixedUpdate()
    {
        ...
        h = Input.GetAxis("Debug Horizontal");
        v = Input.GetAxis("Debug Vertical");
